"""
Admin service (control-plane spec §4.2 sessions, §9 read surfaces; the
mutations land in W2). Every mutation here takes the acting admin and
writes its audit row inside the same transaction; every reader composes
existing services in their read-only form and never creates rows.
"""
...
def get_user_detail(db: Session, user: User, *, today: Optional[date] = None) -> AdminUserDetailResponse:
    ...
    return AdminUserDetailResponse(
        user=AdminUserIdentity(
            ...
            deleted_at=user.deleted_at,
            purge_eligible_at=(
                admin_usage_service.purge_eligible_at(deleted_at)
