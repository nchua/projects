...
| **3 — Load + Guard + Condition v2** | 0b `local_date` and `weight_lb` read switches; §6 service, table, rules, guard; Load strip + sheet; Condition v2; TRIMP into cooldowns; HealthKit background delivery (§9.1, parallel iOS item) | 1–2 | ACWR shows `null` until day 28 then a number; a run appears in Hunt without opening the app |
