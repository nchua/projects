"""Tests for Phase 3: sessions, interleaving, pacing,
anti-guilt, and enhanced summary."""
...
def _setup_topic_with_cards(client, auth_headers, topic_name, cards, db=None):
    ...
    resp = client.post(
        "/api/v1/topics", json={"name": topic_name}, headers=auth_headers
    )
...
def test_interleaving_mixes_topics(client, auth_headers, db):
    ...
    if len(set(topics)) > 1:
        ...
        assert transitions >= 1, (
            "Expected interleaving (topic switches) but cards were grouped"
        )
...
def test_enhanced_summary_per_topic(client, auth_headers, db):
    ...
    _, ai_ids = _setup_topic_with_cards(
        client,
        auth_headers,
        "AI",
        [
    ...
    _, biz_ids = _setup_topic_with_cards(
        client,
        auth_headers,
        "Business",
        [
    ...
    biz_perf = next(
        t for t in data["topic_performance"] if t["topic_name"] == "Business"
    )
