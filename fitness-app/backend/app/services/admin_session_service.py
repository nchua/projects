"""
Admin session mint (control-plane spec §4.2, §4.4): credentials → a
15-minute admin token, with the DB-backed lockout counter and the
``session.create`` audit row. The other admin services:
``admin_read_service`` (§9) and ``admin_mutation_service`` (§7, §8.1);
hard purge is ``purge_service``.
"""
