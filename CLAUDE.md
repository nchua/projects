...
Run `/best-practices` for the Boris Cherny tips (plan mode, CLAUDE.md investment, reusable skills, subagents, prompting, terminal setup). The full list lives in that skill — kept out of this file so it doesn't load ~50 lines into every session's context.
